...
export default function SolarQuoteWizard() {
  ...
  const { formData, updateContact, updateConsumption, updateInstallation } =
    useSolarQuoteForm();
  const { errors, validateStep } = useWizardValidation(
    SOLAR_WIZARD_CONFIG.validation,
  );
  const calculation = useSolarCalculation(tenant);

  const handleNext = async () => {
    const currentStepConfig =
      SOLAR_WIZARD_CONFIG.steps[navigation.currentStep - 1];

    // Validar step actual
    if (!currentStepConfig.skipValidation) {
      const isValid = validateStep(
        navigation.currentStep,
        getStepData(navigation.currentStep),
      );
      if (!isValid) return;
    }
    ...
    if (navigation.currentStep === 3 && !calculation.data) {
      calculation.mutate(formData, {
        onSuccess: () => {
          // Solo avanzar si el cálculo fue exitoso
          navigation.next();
        },
        onError: (error) => {
  ...
  const steps = SOLAR_WIZARD_CONFIG.steps.map((config) => {
    const StepComponent =
      STEP_COMPONENTS[config.component as keyof typeof STEP_COMPONENTS];
        ...
        switch (config.id) {
          ...
          case "results":
            return calculation.data ? (
              <StepResults calculation={calculation.data} />
            ) : null;
          case "cta":
            return calculation.data ? (
              <StepCTA formData={formData} calculation={calculation.data} />
            ) : null;
          default:
            return null;
        }
